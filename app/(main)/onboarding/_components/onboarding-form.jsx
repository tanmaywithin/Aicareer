...
const OnboardingForm = ({ industries, initialData }) => {
  ...
  const {
    ...
    defaultValues: {
      industry: initialData?.industry?.split("-")[0] || "",
      subIndustry: initialData?.industry?.split("-").slice(1).join("-") || "",
  ...
  // Pre-select industry if initialData exists
  useEffect(() => {
    if (initialData?.industry) {
      const industryId = initialData.industry.split("-")[0];
